...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      // Make POST request to the signup API with the provided username and password
      ...
      if (response.status === 200) {
        const responseBody = JSON.parse(response.data.body); // Parse the response body
        setMessage(responseBody.message); // Set the success message in the state
      } else {
